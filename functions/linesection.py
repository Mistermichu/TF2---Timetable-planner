...
def create_block_section(root, messages, post_list):

    def submit_create_block_section():
        sys.path.append(os.path.join(os.path.dirname(__file__), 'functions'))
        from functions import generate_timetable, check_new_file

        folder_name = "line sections"
        block_section_name = str(entry_block_section_name.get())
        track_number = 1
        ...
        try:
            ...
            create_block_section_window.destroy()
            return
        ...
        check_new_file(folder_name=folder_name, file_name=block_section_name,
                       file_data=block_section_data, messages=messages)

        create_block_section_window.destroy()

    create_block_section_window = tk.Toplevel(root)
    create_block_section_window.title(messages["create_block_section"])
    create_block_section_window.resizable(False, False)

    row_value = 0
    # Add line section name
    label_block_section_name = tk.Label(
        create_block_section_window, text=messages["block_section_name"])
    label_block_section_name.grid(row=row_value, column=0, sticky="W")
    entry_block_section_name = tk.Entry(create_block_section_window)
    entry_block_section_name.grid(row=row_value, column=1)

    row_value += 1

    # Add starting point
    label_starting_point = tk.Label(
        create_block_section_window, text=messages["starting_point"])
    label_starting_point.grid(row=row_value, column=0, sticky="w")
    entry_starting_point = ttk.Combobox(
        create_block_section_window, values=post_list.get_names())
    ...
    label_ending_point = tk.Label(
        create_block_section_window, text=messages["ending_point"])
    ...
    label_speed_limit = tk.Label(
        create_block_section_window, text=messages["speed_limit"])
    label_speed_limit.grid(row=row_value, column=0, sticky="W")
    entry_speed_limit = tk.Entry(create_block_section_window)
    entry_speed_limit.grid(row=row_value, column=1)
    label_units = tk.Label(create_block_section_window, text="km/h")
    ...
    label_electrification_status = tk.Label(
        create_block_section_window, text=messages["electrified"])
    ...
    radio_electrification_status_yes = tk.Radiobutton(
        create_block_section_window, text=messages["yes"], variable=var_electrification_status, value="Yes")
    radio_electrification_status_yes.grid(row=row_value, column=1)
    radio_electrification_status_no = tk.Radiobutton(
        create_block_section_window, text=messages["no"], variable=var_electrification_status, value="No")
    ...
    # Submit changes
    row_value += 1
    button_submit = tk.Button(create_block_section_window,
                              text=messages["submit"], command=submit_create_block_section)
